...
**Scope of the κ framework**. κ is computed on the integrator's output embedding `X_integrated`. For integrators producing a native point-embedding (Harmony, scVI, Scanorama, scDREAMER, scCRAFT), this is direct. For graph-only integrators (BBKNN, Seurat-RPCA-via-kNN), an adapter is required; we recommend `X_pca` passthrough (computed once on the pre-integration log-normalized counts and reused post-integration) as the least-confounded choice, since it measures the integrator's displacement of cells without introducing an additional spectral reduction step. Specifically, for BBKNN on Cheng PAAD and Cheng6, X_pca passthrough yields κ_median = 1.27 and 2.03 (Regime A, confirming the prediction that BBKNN preserves kNN structure); a diffmap adapter yields κ_median = 8.4 and 9.3 (artefactually elevated by the spectral reduction). The adapter is therefore part of the integrator specification for the κ framework, not a post-hoc choice; reporting κ without adapter specification is under-determined. This is a documented instance of auxiliary-hypothesis disconfirmation: the core κ prediction remains unfalsified; the auxiliary assumption that downstream adapters are innocuous is falsified. The paper therefore gains a framework-boundary (κ requires pre-specified adapter for graph-only integrators) rather than losing the prediction.

Also scope-limited: the κ three-regime stratification is claimed as a **descriptive empirical tool**, not a predictive theoretical taxonomy. The paper reports that κ stratifies the observed method set (Harmony, Scanorama, scVI, BBKNN-with-X_pca-passthrough) into three regimes on cluster-regime rare-type holdouts, and does NOT claim to predict which regime a previously-unseen method will fall into without direct measurement. For state-regime rare types (canonical TPEX distributed across 6 T-cell motifs in Sade-Feldman), motif-centroid κ does not track the rare-state-preservation signal and the three-regime stratification does not generalize; see `workspace/results/hvg3k_candidate_discrimination.md` for the candidate-A-disconfirmed, candidate-B-supported analysis.
