...
**Falsified cross-method unification**. A 9-point ℓ-weighted regression test (ℓ := fraction of k-NN sharing the cell's coarsened lineage) was run per Math's proposed compartment-locality-amplification hypothesis. Both coarse ℓ (DC/Mac/Mono/Mast/Neut) and fine ℓ (MajorCluster) fail to collapse the 9 points onto a monotone scaling curve in n·π²·Δ²: counter-example pairs exist at (Scanorama PAAD n·π²·Δ²=0.21, ℓ·-log10(p)=2.30) vs (Harmony Cheng5 n·π²·Δ²=2.93, ℓ·-log10(p)=1.13) — the larger-scale point gives half the weighted signal. The compartment-locality mechanism survives as an intra-method explanation for Harmony's below-floor detection (why OT fires despite n·π²·Δ² < 1), but a universal scaling-collapse curve is falsified. See `workspace/results/locality_regression_v1_negative.md` for the 9-point grid, plot, and written disconfirmation.
